...
from app.services.stripe_events import event_object, identifier, load_event
...
_SESSION_ID_MAX = 160
_LOGIN_URL = "/x-autopilot/panel/login"
_PANEL_URL = "/x-autopilot/panel"
...
def _callback_url(request: Request) -> str:
    return f"{_base_url(request)}/x-autopilot/auth/google/callback"


def _login_redirect(error: str | None = None) -> RedirectResponse:
    url = f"{_LOGIN_URL}?error={error}" if error else _LOGIN_URL
    return RedirectResponse(url=url, status_code=303)


def _remember_checkout(request: Request, session_id: str | None) -> str:
    """Keep the Stripe session id in the cookie so Google Sign-In can attach it.

    Clamped: the value comes straight off the query string and the session
    cookie has a size limit.
    """
    clean = (session_id or "").strip()[:_SESSION_ID_MAX]
    if clean:
...
def _format_amount(amount_cents: int, currency: str) -> str:
    """``CHF 149`` / ``CHF 1.50`` — whole francs without the decimals."""
    code = (currency or "chf").upper()
    if amount_cents % 100 == 0:
        return f"{code} {amount_cents // 100}"
    return f"{code} {amount_cents / 100:.2f}"
...
async def success(request: Request, session_id: str = "") -> HTMLResponse:
    ...
    clean = _remember_checkout(request, session_id)
    return render(
        request,
        "x_autopilot/success.html",
        session_id=clean,
...
async def _attach_paid_checkout(db: AsyncSession, checkout_id: str, email: str) -> XAutopilotPlan | None:
    """Bind the Stripe session the buyer arrived with to the Google account they
    signed in with. Best-effort: a Stripe hiccup must not block the sign-in."""
    try:
        session = await retrieve_checkout_session(checkout_id)
        if session_is_paid(session):
            return await upsert_plan_from_checkout(db, session, access_email=email)
    except (StripeError, StripeNotConfigured, ValueError) as exc:
        logger.warning("Could not attach checkout session %s: %s", checkout_id[:40], exc)
    return None
...
async def auth_google_callback(
    ...
    if error:
        return _login_redirect("google_denied")
    if not code or not state:
        return _login_redirect("google_missing")
    try:
        payload = google_oauth.loads_state(state)
        tokens = await google_oauth.exchange_code(code, _callback_url(request))
...
async def panel(request: Request, db: AsyncSession = Depends(get_db)) -> Response:
    ...
    if plan is None and checkout_id:
        plan = await _attach_paid_checkout(db, checkout_id, user["email"])
    ...
    if plan_active:
        ...
        except (GoogleNotConfigured, GoogleOAuthError) as exc:
