...
## Accounts, sourced inventory and direct sellers — 2026-09-13

User direction: preserve the approved design and add login, payments, real offers and a Sell your property path immediately. Implemented free accounts with hashed passwords and single-use recovery keys; persistent private shortlists; four primary-source factual advertisements; free direct submissions with owned photographs; manual publication review; seller enquiry inboxes; optional CHF 49 / 30-day featured placement; owner-only CHF 1 payment check. Stripe fulfilment requires a signed, matching paid event. Availability, ownership, CSRF, duplicate events, wrong amounts and refund/dispute boundaries are enforced and tested.

Worldwide describes the interface and submission capability, not inventory in every country. Source offers are checked on 2026-09-13 and remain subject to advertiser availability. Original descriptions are short factual summaries, with source links and dated EUR estimates. Source photos stay at the original websites. Seller promotion provides labelled placement on Zorbeck, without invented reach or automated external distribution. Detailed operations, sourcing expansion and limitations: `MARKETPLACE.md`.
