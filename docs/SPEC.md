...
Read AGENTS.md if present, the repository README/HANDOVER, this SPEC and TASKS. Read canon _PROTOCOL, 00_INITIATIVES, 02_ASSETS and NEXT with top_only=true; use the newest current entry and current user override, not archived priorities. Resolve existing code/PRs before creating code. Existing components and workflows are the implementation path; do not create a third engine. Preserve unrelated work and source deployments.
...
https://startend.app.n8n.cloud/webhook/agent-report-7q3v9x2k is the bus. GET it for a fresh bus_cursor (TTL 600 seconds); POST {team:"GPT_CURSOR",project:<lane name>,type:"CLAIM",what:"resume Cxx / existing reservation #ID; execute item range",next:"build",bus_cursor:<fresh cursor>}. The existing C01..C32 reservations are #400..#431. A CLAIM does not replace the repo's single-writer check. Refresh the cursor before the final write, never paste an expired cursor from this document.
...
AI owns supported Stripe configuration, products, prices and public payment links, with credential VALUES only in Railway variables. Use a separate CHF 1 live-mode one-time smoke price without replacing commercial prices. Marcin makes the actual self-payment/3DS click. Verify Stripe payment and useful fulfillment, then perform the authorized refund; deactivate/archive the used smoke price/link. A used Stripe Price is archived rather than deleted. Posting a payment link is not proof of payment.
